...
@if ($crud->getFieldsWithoutATab()->filter(function ($value, $key) { return $value['type'] != 'hidden'; })->count())
    <div class="card">
        <div class="card-body row">
            @include('different-core::settings.backpack_overrides.show_fields', ['fields' => $crud->getFieldsWithoutATab()])
        </div>
    </div>
@else
    @include('different-core::settings.backpack_overrides.show_fields', ['fields' => $crud->getFieldsWithoutATab()])
    ...
    <div class="nav-tabs-custom {{ $horizontalTabs ? '' : 'row'}}" id="form_tabs">
        ...
        <div class="tab-content p-0 {{$horizontalTabs ? '' : 'col-md-9'}}">
            @foreach ($crud->getTabs() as $k => $tab)
                <div 
                    role="tabpanel"
                    class="tab-pane {{ isset($tabWithError) ? ($tab == $tabWithError ? ' active' : '') : ($k == 0 ? ' active' : '') }}"
                    id="tab_{{ Str::slug($tab, "") }}"
                >
                    @include('different-core::settings.backpack_overrides.show_fields', ['fields' => $crud->getTabFields($tab)])
                </div>
            @endforeach
        </div>
    </div>
